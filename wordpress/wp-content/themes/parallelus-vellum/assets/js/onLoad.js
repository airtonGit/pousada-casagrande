jQuery(document).ready(function($) {
	...
	var $fullSlider   = $('.top-content-area .type_slide-show').find('.fullscreen-container'),
		...
		fordedFull    = $fullSlider.parent().hasClass('forcefullwidth_wrapper_tp_banner'),
		sliderPad     = 200,
		scrollDelta   = 0,  // wheel inertia
		scrollDir     = 0,
		wpAdminbar    = ($('body.admin-bar').length) ? wpAdminbar = 32 : 0; // Offest for stupid WP adminbar
	...
	if ($fullSlider.length && fordedFull) {
		...
		if (ltIE9) { $('#SliderPad').css('padding-top', $fullSlider.height()+'px'); }
	
		// Fade effect
		// ...................................................................
		$(document).scroll(function() {
			scroll = $(document).scrollTop() - wpAdminbar;
			screenWidth = $(window).width(); // Only use on large screens

			if ( scroll < sliderPad || screenWidth < 768) {
				var percent = scroll/sliderPad, 
					opacity = (screenWidth < 768) ? 1 : 1 - percent || 1;
				$fullSlider.css({'opacity': opacity, 'visibility':'visible', 'left':'0'});
					// if (ltIE9) { $fullSlider.css({'-ms-filter':'"progid:DXImageTransform.Microsoft.Alpha(Opacity='+opacity*100+')"', 'position':'absolute'}); }
				jQuery('window').resize()
				$vMastheadBg.css({'opacity': percent});
					// if (ltIE9) { $vMastheadBg.css({'-ms-filter':'"progid:DXImageTransform.Microsoft.Alpha(Opacity='+opacity*100+')"', 'position':'absolute'}); }
				$('#masthead').addClass('hasSlideShow');
			} else if ( scroll >= sliderPad ) { 
				$fullSlider.css({'opacity': 0, 'visibility':'hidden',  'left':'-200%'});
					// if (ltIE9) { $fullSlider.css({'-ms-filter':'"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)"', 'position':'absolute', 'display':'none'}); }
				$vMastheadBg.css({'opacity': 1});
					// if (ltIE9) { $vMastheadBg.css({'-ms-filter':'"progid:DXImageTransform.Microsoft.Alpha(Opacity=100)"', 'position':'absolute'}); }
	...
	// Docked Top Banner
	// -------------------------------------------------------------------
	if ( dock_topBanner && $(window).width() >= 768 ) {
		docked = false;
		...
		$(window).scroll(function() {
			...
			if ( !docked &&  beginDock <= scroll ) {
				...
				$banner.stop(true).animate({ 'opacity':0, 'top':-$banner.height() }, 200, function() {
	...
	$("a.scrollTop, .scrollTop a, a[href='#ScrollTop']").on("click", function( event ) {
		event.preventDefault();
		$.scrollTo( '#page', 750, {easing: 'easeOutQuart'});
	});
...
// Simple error checking... Prevent blank page from JS error and fade in content effect
// -------------------------------------------------------------------------------------
if ( jQuery('#FadeInContent').is(':visible') ) {
	setTimeout(function() {	jQuery('#FadeInContent').css('display','none'); }, 1500);
}
