...
class bcn_breadcrumb_trail
{
	...
	function do_post()
	{
		...
		if($this->opt['bcurrent_item_linked'] || ( isset($page) && $this->opt['bpaged_display']))
		{
	...
	function display($return = false, $linked = true, $reverse = false)
	{
		...
		foreach($this->trail as $key => $breadcrumb)
		{
			...
			$sep = (strpos($this->opt['hseparator'], 'fa') !== false) ? '&nbsp; <i class="'.$this->opt['hseparator'].'"></i> &nbsp;' : $this->opt['hseparator'];
		...
		$breadcrumbs = '<div class="breadcrumbs">'. $trail_str .'</div>';
	 ...
	 */
	function display_list($return = false, $linked = true, $reverse = false, $list_type = 'ol')
	{
		//Set trail order based on reverse flag
		$this->order($reverse);
		//Initilize the string which will hold the assembled trail
		...
		$breadcrumbs = '<'. $list_type .' class="breadcrumbs">'. $trail_str .'</'. $list_type .'>';
		//Should we return or echo the assembled trail?
		if($return) {
			return $breadcrumbs;
		} else {
